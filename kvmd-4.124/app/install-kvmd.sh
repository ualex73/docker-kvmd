...
pip3 install -U ruamel-yaml --break-system-packages
pip3 install -U luma.core --break-system-packages
pip3 install -U luma.oled --break-system-packages
...
sed -i 's/return self.__inner_get_netconf(int(time.monotonic()) \/\/ 3)\[0\]/return os.getenv("KVMD_IFACE", default=self.__inner_get_netconf(int(time.monotonic()) \/\/ 3)\[0\])/' /usr/lib/python3.13/site-packages/kvmd/apps/oled/sensors.py
[ $? -ne 0 ] && exit 1

# Fix the ip address
sed -i 's/return self.__inner_get_netconf(int(time.monotonic()) \/\/ 3)\[1\]/return os.getenv("KVMD_IPADDR", default=self.__inner_get_netconf(int(time.monotonic()) \/\/ 3)\[1\])/' /usr/lib/python3.13/site-packages/kvmd/apps/oled/sensors.py
[ $? -ne 0 ] && exit 1
...
# 4.107+ moved main.yaml to /usr/lib/kvmd/
cp /usr/lib/kvmd/main.yaml /usr/lib/kvmd/main.yaml.orig
cp /usr/share/kvmd/configs.default/kvmd/main/v4mini-hdmi-rpi4.yaml /usr/lib/kvmd/main.yaml
...
cat <<EOF >> /etc/kvmd/override.d/00-default.yaml
kvmd:
...
ls -l src/transports/.libs/libjanus_websockets.so*
cp src/transports/.libs/libjanus_websockets.so.2.0.8 /usr/lib/janus/transports/
[ $? -ne 0 ] && exit 1
ln -s /usr/lib/janus/transports/libjanus_websockets.so.2.0.8 /usr/lib/janus/transports/libjanus_websockets.so.2
ln -s /usr/lib/janus/transports/libjanus_websockets.so.2.0.8 /usr/lib/janus/transports/libjanus_websockets.so
